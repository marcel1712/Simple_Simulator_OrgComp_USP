	...
	switch(state)
	{
		...
		case STATE_DECODE:
			...
			switch(opcode){
				...
				case LOAD:
					...
					//LOAD R1 1000 carregar o valor da posição de memória 1000 no registrador R1
					// MAR = MEMORY[PC];
					// PC++;
					selM1 = sPC;//le a linha do pc
					RW = 0;//modo de leitura
					LoadMAR = 1;//permite entrar o no mar o endereço de memoria do load
					IncPC = 1;//incrementa o pc para ir para proxima linha
					// -----------------------------
					state=STATE_EXECUTE;
					break;

				case STORE:
					//transfere dados de um registrador 
					//para um endereço específico da memória 
					//STORE R1, 2000
					//MAR = MEMORY[PC];
					//PC++;
					selM1 = sPC;//permite a posicao do pc ler a posicao de memoria
					RW = 0;//le o endereco  apontado e joga para fora
					LoadMAR = 1;//coloca o valor do endereço para dentro do MAR
					// -----------------------------
					state=STATE_EXECUTE;
					break;
				...
				case LOADI:
					...
					selM4 = ry;//permite o dado do resitrador y passar pelo selM4
					selM1 = sM4;//procurar na memoria o endereço do ry na memória
					RW = 0;//leitura e faz o dado do endereço ry sair
					selM2 = sDATA_OUT;//permite o dado que saiu da memoria peassa pelo mux2 indo em direção registrador
					LoadReg[rx] = 1;//permite o valor que saiu da memoria do endereço do ry entrar em outro registrador rx
					// -----------------------------
					state=STATE_FETCH;
					break;
				...
				case STOREI:
					//mem[reg[rx]] = reg[ry];
					selM4 = rx;//permite o dado sair do registrador passar pelo m4
					selM1 = sM4;//permite o valo do rx entrar como endereço de memoria
					RW = 1;//escreve na linha de endereço
					selM3 = ry;//permite o dado do registrador passar pela m3
					selM5 = sM3;//permite o dado do registrador ser escrito na memoria
					// -----------------------------
					state=STATE_FETCH;
					break;

				case MOV:
					//copia dados entre registradores, entre a memória e registradores, ou de valores imediatos para registradores
				...
				case CALL:
					...
					if((COND == 0)                       	                      // NO COND
							...
							|| (FR[9]==1 && (COND==13)))                          // DIVBYZERO
					{ // PC = MEMORY[PC];
						//copiar o valor do PC para a pilha
						selM5 = sPC;
						RW = 1;
						selM1 = sSP;
						DecSP = 1;
				...
				case PUSH:
				//Push faz os valores dos registradores ou endereços de retorno podem ser "empilhados" para serem restaurados posteriormente.
				//SP tem registrado os endereços de memoria guardados em pilha
					selM1 = sSP;//permite o valor do SP entrar para endereçar a memoria
					RW = 1;//escreve nesse endereço
					selM3 = rx;//permite o valor do registrador passar
					selM5 = sM3;//coloca o valor do registrador dentro do endereço criado
					DecSP = 1;//decrementando o SP para subir um valor da pilha e apontar uma "caixa vazia"
					// -----------------------------
					state=STATE_FETCH;
					break;
				...
				case RTS:
					// SP++;
					IncSP = 1;//para ler o stackpointer temos q incrementar 1;
					// -----------------------------
					state=STATE_EXECUTE;
					break;
		...
		case STATE_EXECUTE:
			...
			switch(opcode){
				case LOAD:
					//reg[rx] = MEMORY[MAR];
					selM1 = sMAR;//vai para linha do endereço do load
					RW = 0;//lê o valor dessa linha e manda para fora da memoria
					selM2 = sDATA_OUT;//deixa passar no mux 2 caminho do registrador
					LoadReg[rx] = 1;//permite a entrada no valor no registrado rx
					// -----------------------------
					state=STATE_FETCH;
					break;
				...
				case STORE:
					//MEMORY[MAR] = reg[rx];
					selM1 = sMAR;//
					RW = 1;//escreve no endereço do mar
					selM3 = rx;//permite o dado do registrador passar pelo mux3
					selM5 = sM3;//permite o dado que passou pelo m3 passar pelo m5 entrar na memoria
					// -----------------------------
					state=STATE_FETCH;
					break; 
				...
				case CALL:
					selM1 = sPC;
					RW = 0;
					LoadPC = 1;//deixando o endereco q esta na entrada no caso a posicao da funcao na memoria
		...
		case STATE_EXECUTE2:
			//só o case RTS vai mandar para o executa 2, entao nao precisa criar os cases
			//case RTS:
			//PC++;
			IncPC = 1;
			// -----------------------------
